...
double max_velocity = 20; //won't send velocity data if exceed max_velocity
        ...
        if ( Robot ->header.seq != 0){
            ...
            for( int i = 0; i < Robot -> robots.size(); i++){
                if ( previous_status[Robot -> robots[i].id] == 1){ // it means that i have seen more than marker of this robot
                    ...
                    distance_calculate_total = sqrtf128(powf64x(Robot -> robots[Robot -> robots[i].id].pose.position.x - previous_pose_array[Robot -> robots[i].id].position.x, 2) + 
                        powf64x(Robot -> robots[Robot -> robots[i].id].pose.position.y - previous_pose_array[Robot -> robots[i].id].position.y, 2) + 
                        powf64x(Robot -> robots[Robot -> robots[i].id].pose.position.z - previous_pose_array[Robot -> robots[i].id].position.z, 2));
                    distance_xy = sqrtf128(powf64x(Robot -> robots[Robot -> robots[i].id].pose.position.x - previous_pose_array[Robot -> robots[i].id].position.x, 2) + 
                        powf64x(Robot -> robots[Robot -> robots[i].id].pose.position.y - previous_pose_array[Robot -> robots[i].id].position.y, 2));
                    velocity = distance_calculate_total / (duration.nsec * pow(10, -9)); 
                    velocity_x = (Robot -> robots[Robot -> robots[i].id].pose.position.x - previous_pose_array[Robot -> robots[i].id].position.x )/ (duration.nsec * pow(10, -9)); 
                    velocity_y = (Robot -> robots[Robot -> robots[i].id].pose.position.y - previous_pose_array[Robot -> robots[i].id].position.y )/ (duration.nsec * pow(10, -9)); 
                    velocity_z = (Robot -> robots[Robot -> robots[i].id].pose.position.z - previous_pose_array[Robot -> robots[i].id].position.z )/ (duration.nsec * pow(10, -9)); 
                    ...
                    ROS_INFO("distance [%f] distance_xy [%f] velocity [%f]", distance_calculate_total, distance_xy, velocity);
                    if (velocity_xy < max_velocity){
